...
instance HasDiff Int
instance HasDiff Text
...
instance (Ord k, HasDiff v) => HasDiff (Map.Map k v) where
  ...
  diff old new = case F.fold $ Map.merge deletes inserts mDiffs old new of
                   Updates [] [] [] -> NoDifference
                   diffs            -> Difference diffs
    where
      deletes = Map.mapMissing $ \k v -> Updates []      [(k,v)] []
      inserts = Map.mapMissing $ \k v -> Updates [(k,v)] []      []
      mDiffs  = Map.zipWithMaybeAMatched $ \k old' new' -> Identity $
                      case diff old' new' of
                        NoDifference      -> Nothing
                        Difference delta  -> Just $ Updates [] [] [(k,delta)]
